...
def createModelAndTrain():
    ...
    autoencoder_train = autoencoder.fit(Atac, Atac, batch_size=256, epochs=nEpochs,validation_data=(Atac, Atac),callbacks=callbacks_list)
    ...
    encWeights=pd.DataFrame(autoencoder.weights[0])
    encWeights.index=Atac.columns
    encWeights=encWeights.T
    encWeights.index=relationMatrix.columns
    encWeights=encWeights.T
    decWeights=pd.DataFrame(autoencoder.weights[2])
    decWeights.index=relationMatrix.columns
    decWeights=decWeights.T
    decWeights.index=Atac.columns
    decWeights=decWeights.T
    ds.T.to_csv("./Results/"+projectName+"/permutation/"+str(nPerm)+"denseSpace"+extension,sep=sep)
    encWeights.to_csv("./Results/"+projectName+"/weights/"+str(nPerm)+"enc_weights"+extension,sep=sep)
    decWeights.to_csv("./Results/"+projectName+"/weights/"+str(nPerm)+"dec_weights"+extension,sep=sep)
...
if bias == "TF":
    ...
    Atac=Atac[Atac.T.index.intersection(tf.iloc[:,1])]
...
if bias == "mirna":
    ...
    tf=pd.read_csv(tfName,sep="\t",header=0)
    Atac=Atac[Atac.T.index.intersection(tf.iloc[:,3])]
    transcriptionFactors=np.unique(tf.iloc[:,0])
    relationMatrix=pd.DataFrame(np.zeros((Atac.columns.shape[0],transcriptionFactors.shape[0])))
    relationMatrix.index=Atac.columns
    relationMatrix.columns=transcriptionFactors
...
if bias == "cytoBands":
    ...
    tf=pd.read_csv(tfName,sep=",",header=0)
    Atac=Atac[Atac.T.index.intersection(tf.iloc[:,0])]
...
try:
    os.mkdir("./Results/"+projectName+"/weights")
except OSError:
    print ("Creation of the directory %s failed" % "./Results/"+projectName+"/weights")
else:
    print ("Successfully created the directory %s " % "./Results/"+projectName+"/weights")
